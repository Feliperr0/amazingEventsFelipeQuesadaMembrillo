...
function generateCategoryCheckboxes(data, container) {
  // Extract unique categories using a Set
  const categorias = new Set(data.events.map((event) => event.category));

  // Build the checkbox HTML string
  let checkboxesHTML = "";
  for (const category of categorias) {
    const contenedor = document.createElement("div")
    contenedor.className = "navbar-nav me-auto mb-2 mb-lg-0";
    checkboxesHTML += `
      <div class="form-check m-1">
        <input class="category-filter" type="checkbox" value="${category}" id="checkbox-${category.replace(" ", "-")}"> 
        <label class="form-check-label" for="checkbox-${category.replace(" ", "-")}"> ${category}</label>
      </div>`;
  }
...
// Initial rendering
generateCategoryCheckboxes(data, checkboxContainer);
pintarTarjetas(data.events); // Replace with your function to display events
